...
#define COLOR_END   "\033[0m"
    ...
    private:
        ...
        )
        {
            if( failed + succeeded == 0 )
                m_config.stream() << COLOR_INFO << "No tests ran" << COLOR_END;
            else if( failed == 0 )
                m_config.stream() << COLOR_OK << "All " << succeeded << " test(s) succeeded" << COLOR_END;
            else if( succeeded == 0 )
                m_config.stream() << COLOR_FAIL << "All " << failed << " test(s) failed" << COLOR_END;
            else
                m_config.stream() << COLOR_OK << succeeded << " test(s) passed " << COLOR_FAIL << "but " << failed << " test(s) failed" << COLOR_END;
        }
    ...
    private: // IReporter
        ...
        )
        {
            ...
            StartSpansLazily();
	    if (!resultInfo.ok()) m_config.stream() << COLOR_FAIL;
        ...
        void StartSpansLazily()
        {
            if( !m_testingSpan.emitted )
            {
                if( m_config.getName().empty() )
                    m_config.stream() << COLOR_INFO << "[Started testing]" << COLOR_END << std::endl;
                else
                    m_config.stream() << COLOR_INFO << "[Started testing: " << m_config.getName() << "]" << COLOR_END << std::endl;
                m_testingSpan.emitted = true;
            }

            if( !m_groupSpan.emitted && !m_groupSpan.name.empty() )
            {
                m_config.stream() << COLOR_INFO << "[Started group: '" << m_groupSpan.name << "']" << COLOR_END << std::endl;
                m_groupSpan.emitted = true;
            }

            if( !m_testSpan.emitted )
            {
                m_config.stream() << std::endl << COLOR_INFO << "[Running: " << m_testSpan.name << "]" << COLOR_END << std::endl;
